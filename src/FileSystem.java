...
public class FileSystem{
	...
	// admin - Danny abc123456 
	// user - Kenny abc123456
        ...
        if(args.length == 1){
                ...
                if(!exist(username)){
                    ...
                    if(confirmPassword.equals(password)){
                        if(isPasswordStrong(password)){
                            ...
                            writeToFile(username+":"+salt,"db\\salt.txt");
                            String passSalt = password+salt;
                            String hash = toHash(passSalt);
                            writeToFile(username+":"+hash+":"+"A","db\\shadow.txt");
            ...
            if(!salt.equals("")){
                ...
                if(hash.trim().equals(hashFromShadow.trim())){
                    loadSettingsFromFile(); //read from db\settings.txt
                    loadStudentsFromFile(); //read from db\students.txt
                    loadCoursesFromFile(); //read from db\courses.txt
                    //user logged in
                    String userType = getUserTypeFromShadow(username);
                    if(userType.charAt(0) == 'S'){ //Student
    ...
    private static void swopIndexNumberWithAnother(Scanner sc){
        ...
        if(!salt.equals("")){
            ...
            if(hash.trim().equals(hashFromShadow.trim())){
                ...
                overWriteWholeFile(lines, "db\\courses.txt");
        ...
        if(getVacancy(courseIndex) != 9999){ //if there is a slot available
            if(getVacancy(courseIndex) != 0 ){
                if(!checkIfStudentIsTakingCourse(currentStudent.getMetricNumber(), crs.getCourseCode())){ //check if student is already taking this course
                    ...
                    ArrayList<String> lines = convertCoursesIntoLines();
                    //overwrite current db\course.txt file
                    overWriteWholeFile(lines, "db\\courses.txt");
    ...
    private static void addCourse(Scanner sc){
        ...
        writeToFile(courseString,"db\\courses.txt");
        courses.add(c);
        printCourses();
    }
            ...
            if(confirmPassword.equals(password)){
                if(isPasswordStrong(password)){
                    ...
                    if(!studentExists(s)){
                        ...
                        writeToFile(username+":"+metric+":"+gender+":"+nationality,"db\\students.txt");
                        writeToFile(username+":"+salt,"db\\salt.txt");
                        String passSalt = password+salt;
                        String hash = toHash(passSalt);
                        writeToFile(username+":"+hash+":"+"S","db\\shadow.txt");
    ...
    private static void editStudentAccessPeriod(Scanner sc) {
        ...
        if(startTimeIndex == 9999 && endTimeIndex == 9999) {
            ...
            overWriteWholeFile(settings, "db\\settings.txt");
    ...
    private static void loadCoursesFromFile() {
        courses.clear();
        try {
            FileReader reader = new FileReader("db\\courses.txt");
            int character;
            String line = "";
    ...
    private static void loadStudentsFromFile() {
        students.clear();
        try {
            FileReader reader = new FileReader("db\\students.txt");
            int character;
            String line = "";
            while ((character = reader.read()) != -1) {
                if((char) character == '\n'){
    ...
    private static void loadSettingsFromFile() {
        settings.clear();
        try {
            FileReader reader = new FileReader("db\\settings.txt");
            int character;
            String line = "";
            while ((character = reader.read()) != -1) {
                if((char) character == '\n'){
    ...
    private static String getUserTypeFromShadow(String username){
        try {
            FileReader reader = new FileReader("db\\shadow.txt");
            int character;
            String line = "";
            while ((character = reader.read()) != -1) {
                if((char) character == '\n'){
    ...
    private static String getSalt(String username){
        try {
            FileReader reader = new FileReader("db\\salt.txt");
            int character;
            String line = "";
            while ((character = reader.read()) != -1) {
                if((char) character == '\n'){
